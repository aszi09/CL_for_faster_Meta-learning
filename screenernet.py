...
from NP_Trainer import NP_Trainer
...
class ScreenerNet():
    ...
    def data_prepare(self, loader):
        ...
        self.weights = jnp.zeros(self.data_size)
    ...
    def data_curriculum(self, loader):
        ...
        L = Variable(jnp.zeros(1), requires_grad=True).to(self.device)
        for i, j in zip(l, w):
    ...
    def loss_curriculum(self, criterion, outputs, labels, indices):
        return jnp.mean(criterion(outputs, labels) * self.weights[indices])


class ScreenerNetTrainer():
    def __init__(self, data_name, net_name, device_name, num_epochs, random_seed):
        cl = ScreenerNet()

        self.trainer = NP_Trainer(
            data_name, net_name, device_name, num_epochs, random_seed,
            cl.name, cl.data_prepare, cl.model_prepare,
            cl.data_curriculum, cl.loss_curriculum)
    def fit(self):
